import Glimpses_1 from "../Components/Images/Glimpses_1.jpg";
import Glimpses_2 from "../Components/Images/Glimpses_2.jpg";
import Glimpses_3 from "../Components/Images/Glimpses_3.jpg";
import JayantSir from "../Components/Images/JayantSir.jpg";
import Pune_img_1 from "../Components/Images/Pune_img_1.jpg";
import Pune_img_2 from "../Components/Images/Pune_img_2.jpg";
import Pune_img_3 from "../Components/Images/Pune_img_3.jpg";
import Pune_img_4 from "../Components/Images/Pune_img_4.jpg";
import Pune_img_5 from "../Components/Images/Pune_img_5.jpg";
import Pune_img_6 from "../Components/Images/Pune_img_6.jpg";
import Pune_img_7 from "../Components/Images/Pune_img_7.jpg";
import Pune_img_8 from "../Components/Images/Pune_img_8.jpg";
import Pune_img_9 from "../Components/Images/Pune_img_9.jpg";
import Alumni_2017_img_1 from "../Components/Images/Alumni_2017_img_1.jpg";
import Alumni_2017_img_2 from "../Components/Images/Alumni_2017_img_2.jpg";
import Alumni_2017_img_3 from "../Components/Images/Alumni_2017_img_3.jpg";
import Alumni_2017_img_4 from "../Components/Images/Alumni_2017_img_4.jpg";
import Alumni_2017_img_5 from "../Components/Images/Alumni_2017_img_5.jpg";
import Alumni_2017_img_6 from "../Components/Images/Alumni_2017_img_6.jpg";
import Alumni_2017_img_7 from "../Components/Images/Alumni_2017_img_7.jpg";
import Alumni_2017_img_8 from "../Components/Images/Alumni_2017_img_8.jpg";
import Alumni_2017_img_9 from "../Components/Images/Alumni_2017_img_9.jpg";
import React from 'react'
import './AlumniActivities.css';
import Header from './Header'
import Navbar from './Navbar'

function AlumniActivities() {
  return (
    <div>
      <Header />
      <Navbar />

      <div class="col-6">
        <div class="Th-1">
          <div class="Alumni-img">
            <img id="JayantSir" src={JayantSir} class="Jayantsir-img" alt="JayantSir" />
          </div>
          <h2 id="Incharge-Name">MR. JAYANT ROHANKAR</h2>
          <p id="Incharge">Amlumni Incharge</p>
          <p id="Incharge"> 85549 82302</p>
          <p id="Incharge"> jayant.mtech@tgpcet.com </p>
          <div class="Alumni-line"></div>
          <ul class="AlumniSocial-Icons">
            <li class="facebook">
              <a><i class="fab fa-facebook-f"></i></a>
            </li>
            <li class="youtube">
              <a><i class="fab fa-youtube"></i></a>
            </li>
          </ul>
        </div>
      </div>

      <div class="Alumni-panel">
        <h2> Alumni Meet (Nostalgia)</h2>
        <h3 class="panel-heading">Tulsiramji Gaikwad-Patil College of Engineering & Technology Alumni Association Registration Number:- MAH 701/14(Nag)</h3>
        <p class="panel-detail"> Nostalgia is the flagship event of the Alumni association of TGPCET. It is the annual Alumni Meet - an evocative event that marks home coming, celebration and unparalleled joy of communion with the alma mater. It is an ideal opportunity to renew old bonds, foster new ties and relive the magical moments all over again. The Alumnus series is a new initiative launched, which provides an exciting platform for regular interaction between the current students & the alumni of TGPCET, facilitated by Guest Lectures, Technical Events, Workshops, etc.</p>
        <p class="panel-detail">TusiramjiGaikwad-Patil College of Engineering & Technology Alumni network brings together graduates of a variety of programs right from its inception. The collective strength of this alumni network is the backbone of TGPCET. The alumni association acts as a bridge between the illustrious alumni and their alma mater, giving the current students and the alumni a chance to develop positive synergies and grow together in this process. The strength of any institute is comprises the present students, who are the future leaders and the alumni of the college, who bring in accolades by virtue of their success and standing in the society. It gives an opportunity for the present, the past and the incoming batch to interact with each other and learn from shared experiences.</p>
      </div>

      <div class="Alumni-panel2">
        <h2> Glimpses of Online Alumni Meet 2019-20 </h2>
        {/* <div class="panel2-row">
          <div class="panel2-imgbox">
            <div class="thumbnail">
              <img src="http://tgpcet.com/assets/img/ALUMNI/Alumni-Meet-2019-20/1.jpg" class="panel2-img" alt="img" />
            </div>
          </div>
          <div class="panel2-imgbox">
            <div class="thumbnail">
              <img src="http://tgpcet.com/assets/img/ALUMNI/Alumni-Meet-2019-20/2.jpg" class="panel2-img" alt="img" />
            </div>
          </div>
          <div class="panel2-imgbox">
            <div class="thumbnail">
              <img src="http://tgpcet.com/assets/img/ALUMNI/Alumni-Meet-2019-20/3.jpg" class="panel2-img" alt="img" />
            </div>
          </div>
        </div> */}

        <div id="panel2-row">
          <div class="panel2-img">
            <img id="Glimpses_1" src={Glimpses_1} alt="Glimpses_1" />
            <p class="caption"> Alumni, Treasurer, Principal, Vice Principal and Heads present during Online Alumni Meet</p>
          </div>
          <div class="panel2-img">
            <img id="Glimpses_2" src={Glimpses_2} alt="Glimpses_2" />
            <p class="caption"> Alumni of Institute sharing their views about Nationwide lockdown due to COVID-19 during Online Alumni Meet</p>
          </div>
          <div class="panel2-img">
            <img id="Glimpses_3" src={Glimpses_3} alt="Glimpses_3" />
            <p class="caption">Mr. Amab Singha, 2012 Batch, TCS Workforce Management, Data Analyst, Department of Computer Science & Enginnering sharing views about Nationwide lockdown due to COVID-19 during Online Alumni Meet </p>
          </div>
        </div>
      </div>


      <div class="Alumni-panel2">
        <h2>Alumni Meet at Pune (Nostalgia 2018)</h2>

        <div id="panel2-row">
          <div class="panel2-img">
            <img
              id="Pune_img_1" src={Pune_img_1} alt="Pune_img_1"
            />
            <p class="caption">
              Lighting of Ceremonial Lamp with the hands of Hon'ble Principal
              Dr.G.K. Awari
            </p>
          </div>
          <div class="panel2-img">
            <img
              id="Pune_img_2" src={Pune_img_2} alt="Pune_img_2"
            />
            <p class="caption">
              Hon'ble Principal Dr.G.K. Awari addressing to alumni
            </p>
          </div>
          <div class="panel2-img">
            <img
              id="Pune_img_3" src={Pune_img_3} alt="Pune_img_3"
            />
            <p class="caption">
              Hon'ble Prof. Sandeep Gaikwad, Treasurer, GPGI Felicitating Mr.
              Rahul Naidu as Alumni Coordinator of TGPCET in Pune & Mumbai Region.
            </p>
          </div>
        </div>

        <div id="panel2-row">
          <div class="panel2-img">
            <img
              id="Pune_img_4" src={Pune_img_4} alt="Pune_img_4"
            />
            <p class="caption">
              Dr.G.K. Awari, Principal, TGPCET Felicitating Ms. Ashwini Kulkarni
              as Alumni Coordinator of TGPCET in Pune & Mumbai Region.
            </p>
          </div>
          <div class="panel2-img">
            <img
              id="Pune_img_5" src={Pune_img_5} alt="Pune_img_5"
            />
            <p class="caption">
              Alumni share their views about the achievement of TGPCET
            </p>
          </div>
          <div class="panel2-img">
            <img
              id="Pune_img_6" src={Pune_img_6} alt="Pune_img_6"
            />
            <p class="caption">
              Alumni share their views about the achievement of TGPCET
            </p>
          </div>
        </div>

        <div id="panel2-row">
          <div class="panel2-img">
            <img
              id="Pune_img_7" src={Pune_img_7} alt="Pune_img_7"
            />
            <p class="caption">
              Alumni share their views about the achievement of TGPCET
            </p>
          </div>
          <div class="panel2-img">
            <img
              id="Pune_img_8" src={Pune_img_8} alt="Pune_img_8"
            />
            <p class="caption">Memories of Alumni Meet (Nostalgia-18) at Pune</p>
          </div>
          <div class="panel2-img">
            <img
              id="Pune_img_9" src={Pune_img_9} alt="Pune_img_9"
            />
            <p class="caption">Memories of Alumni Meet (Nostalgia-18) at Pune</p>
          </div>
        </div>
      </div>

      <div class="Alumni-panel2">
        <h2>Alumni Meet 2017</h2>

        <div id="panel2-row">
          <div class="panel2-img">
            <img
              id="Alumni_2017_img_1" src={Alumni_2017_img_1} alt="Alumni_2017_img_1"
            />
            <p class="caption">
              Lighting of Ceremonial Lamp with the hands of Chief Guest
            </p>
          </div>
          <div class="panel2-img">
            <img
              id="Alumni_2017_img_2" src={Alumni_2017_img_2} alt="Alumni_2017_img_2"
            />
            <p class="caption">Inauguration of Alumni Meet</p>
          </div>
          <div class="panel2-img">
            <img
              id="Alumni_2017_img_3" src={Alumni_2017_img_3} alt="Alumni_2017_img_3"

            />
            <p class="caption">
              Welcome of Chief Guest by Prof. Sandeep Gaikwad, Treasurer, GPGI
            </p>
          </div>
        </div>

        <div id="panel2-row">
          <div class="panel2-img">
            <img
              id="Alumni_2017_img_4" src={Alumni_2017_img_4} alt="Alumni_2017_img_4"
            />
            <p class="caption">Chief Guest addressing Alumni of Institute</p>
          </div>
          <div class="panel2-img">
            <img
              id="Alumni_2017_img_5" src={Alumni_2017_img_5} alt="Alumni_2017_img_5"
            />
            <p class="caption">Department of Mechanical Engineering</p>
          </div>
          <div class="panel2-img">
            <img
              id="Alumni_2017_img_6" src={Alumni_2017_img_6} alt="Alumni_2017_img_6"
            />
            <p class="caption">Department of Computer Science & Engineering</p>
          </div>
        </div>

        <div id="panel2-row">
          <div class="panel2-img">
            <img
              id="Alumni_2017_img_7" src={Alumni_2017_img_7} alt="Alumni_2017_img_7"
            />
            <p class="caption">Department of Computer Science & Engineering</p>
          </div>
          <div class="panel2-img">
            <img
              id="Alumni_2017_img_8" src={Alumni_2017_img_8} alt="Alumni_2017_img_8"
            />
            <p class="caption">Department of Electronics Engineering</p>
          </div>
          <div class="panel2-img">
            <img
              id="Alumni_2017_img_9" src={Alumni_2017_img_9} alt="Alumni_2017_img_9"
            />
            <p class="caption">
              Department of Electronics & Communication Engineering
            </p>
          </div>
        </div>
      </div>




    </div>
  )
}

export default AlumniActivities